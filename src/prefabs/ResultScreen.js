...
class ResultScreen extends Phaser.GameObjects.Container {
	...
	constructor(scene, x, y) {
		super(scene, x ?? 4, y ?? -57);

		// player_name_board
		const player_name_board = scene.add.image(0, 57, "Player-name-board");
		player_name_board.scaleX = 0.7;
		player_name_board.scaleY = 0.7;
		this.add(player_name_board);

		// playerName
		const playerName = scene.add.text(-241, 55, "", {});
		...
		// playerScore
		const playerScore = scene.add.text(34, 55, "", {});
		...
		// playerWinning
		const playerWinning = scene.add.text(248, 55, "", {});
	...
	setWinnerData = (data) => {
		...
		this.playerName.text = data.sUserName;
